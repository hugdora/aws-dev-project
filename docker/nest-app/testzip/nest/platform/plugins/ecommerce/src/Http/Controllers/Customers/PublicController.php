    ...
    public function getDownload(int|string $id, Request $request, BaseHttpResponse $response)
    {
        ...
        if ($request->input('external')) {
            if ($externalProductFiles->count()) {
                ...
                return Theme::scope('ecommerce.download-external-links', compact('orderProduct', 'product', 'externalProductFiles'), 'plugins/ecommerce::themes.download-external-links')
                    ->render();
            }
